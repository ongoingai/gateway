...
ongoingai debug [last] [--config PATH] [--trace-id ID] [--format text|json] [--limit N] [--include-headers] [--include-bodies]
```
...
- Resolves latest trace (or `--trace-id`) and finds related lineage checkpoints.
