...
| `observability.otel.prometheus_path` | string | `/metrics` | No | Path for the Prometheus scrape endpoint. Must start with `/` and must not overlap with `/api`, `/openai`, or `/anthropic`. |
