...
description: Query usage and estimated cost from trace data with tenant-scoped analytics endpoints.
related:
  title: Next steps
...
Use this page to understand how analytics endpoints compute usage and estimated
cost from stored traces.
You can use these endpoints for spend visibility, trend analysis, and capacity
planning.
...
5. Summary endpoint combines usage, cost, model, and key aggregates in one
   response.
...
- Fix: Use supported values:
  `group_by=provider|model`, `bucket=hour|day|week`, and `to >= from`.
